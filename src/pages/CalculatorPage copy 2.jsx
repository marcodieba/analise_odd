...
import { getCache, setCache } from '../utils/cache.js'; // Importa as funções de cache
...
export default function CalculatorPage({ onRegisterBet, analysisState, updateAnalysisState }) {
    // --- CHAVES DAS APIs (Lidas do .env.local) ---
    const apiKey = import.meta.env.VITE_API_FOOTBALL_KEY; 
    const gnewsApiKey = import.meta.env.VITE_GNEWS_KEY;
    
    // --- Desestruturação do estado recebido por props ---
    const {
    ...
    // --- Estados locais que não precisam ser globais ---
    ...
    useEffect(() => {
        // Apenas busca dados se um jogo for selecionado E ainda não tivermos os resultados para ele.
    ...
    // --- Handler para a mudança de Liga ---
    ...
    const apiFetch = async (endpoint, isCritical = true) => {
        ...
        try {
            ...
            await setCache(cacheKey, responseData); // Salva no cache
            return responseData;
        } catch (err) {
            const errorMessage = `Erro na API: ${err.message}.`;
            console.error(errorMessage, `Endpoint: ${endpoint}`);
            if (isCritical) {
                updateAnalysisState({ error: errorMessage });
            }
            return null;
        } finally {
    ...
    const fetchFixtures = async (leagueId) => { 
        ...
        if (data) {
            updateAnalysisState({ fixtures: data });
        }
    };
    ...
    const analyzeStyleClash = (styleData) => {
        ...
        } else if (away.possession > 60 && home.possession < 40 && home.conversionRate > 0.15) {
            clashResult = {
                signal: 'alert',
                title: 'Alerta: Domínio vs. Contra-Ataque',
        ...
        } else if (home.shotsPerGame < 10 && away.shotsPerGame < 10 && (home.possession > 45 && home.possession < 55)) {
            clashResult = {
                signal: 'confirm',
        ...
        updateAnalysisState({ styleClash: clashResult });
    };
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <main className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
                            <div className="bg-gray-700/50 p-4 rounded-lg mb-6">
                                ...
                                {!apiKey ? (
                                    ...
                                    <div className="flex flex-col space-y-4">
                                        <select value={selectedLeague} onChange={(e) => handleLeagueChange(e.target.value)} className="w-full bg-gray-800 border border-gray-600 rounded-md px-3 py-2 text-white">
                                        ...
                                        <select 
                                            ...
                                            onChange={(e) => {
                                                const newFixtureId = e.target.value;
                                                // Apenas limpa os resultados se o ID do jogo realmente mudar
                                                if (newFixtureId !== selectedFixtureId) {
                                                    updateAnalysisState({ 
                                                        selectedFixtureId: newFixtureId,
                                                        results: null 
                                                    });
                                                } else {
                                                    updateAnalysisState({ selectedFixtureId: newFixtureId });
                                                }
                                            }} 
                            ...
                            <div className="mt-6">
                                <button onClick={runAutoAnalysis} disabled={!results || loading} className="w-full bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded-lg text-lg disabled:opacity-50 disabled:cursor-not-allowed">
                                    {loading ? 'A Analisar...' : 'Executar Auto-Análise ⚡'}
                                </button>
                            </div>
